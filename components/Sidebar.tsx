...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, brand, onEditBrand }) => {
  const menuItems = [
  ...
  return (
    <aside className="w-64 h-screen bg-[#1A2E24] text-[#FDFBE2] flex flex-col fixed left-0 top-0 shadow-2xl z-40 border-r border-[#ffffff08]">
      <div className="p-8 pb-10 flex flex-col items-center gap-4 relative group">
        
        <button 
          onClick={onEditBrand}
          className="absolute top-4 right-4 p-2 bg-[#6B8E23] text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all shadow-xl hover:scale-110 z-50"
          title="Editar Marca"
        >
          <Edit3 size={14} />
        </button>
        ...
        <div className="w-full flex flex-col items-center cursor-pointer" onClick={onEditBrand}>
          <h2 className="text-2xl font-black text-[#FDFBE2] tracking-tighter uppercase leading-none break-words px-2 text-center w-full">
            {brand.name}
          </h2>
          <div className="w-full flex justify-center">
            <p className="text-xs font-black text-[#6B8E23] tracking-[0.4em] uppercase mt-1.5 mr-[-0.4em] text-center w-full">
              {brand.slogan}
            </p>
          </div>
        </div>
